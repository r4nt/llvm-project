...
void printLine(llvm::raw_ostream &OS, const UnwrappedLine &Line,
  ...
  for (std::list<UnwrappedLineNode>::const_iterator I = Line.Tokens.begin(),
    ...
    OS << I->Tok->Tok.getName() << "["
       << "T=" << I->Tok->getType() << ", OC=" << I->Tok->OriginalColumn
       << ", \"" << I->Tok->TokenText << "\"] ";
...
UnwrappedLineParser::UnwrappedLineParser(
    ...
    : Line(new UnwrappedLine), MustBreakBeforeNextToken(false),
      ...
      Macros(Style.Macros, SourceMgr, Style, Encoding, Allocator, IdentTable) {}
...
bool UnwrappedLineParser::containsExpansion(const UnwrappedLine &Line) {
  for (const auto &N : Line.Tokens) {
    if (N.Tok->MacroCtx.Role != MR_None)
      return true;
...
void UnwrappedLineParser::addUnwrappedLine() {
  ...
  if (CurrentLines == &Lines && !InExpansion && containsExpansion(*Line)) {
    if (!Unexpand) {
      Unexpand = std::make_unique<Unexpander>(Line->Level, Unexpanded);
    }
